...
#    * Asset filenames that don't match any of the 8 supported target keys.
...
# The 8 targets that are supported by `scripts/_lib.sh` and listed in the
# README. The script only generates entries for whatever the releases
# actually have — so if `darwin-amd64` isn't published yet, it's simply
# absent from the manifest for that OTP version. Anything else (legacy
# naming like `amd64-glibc.tar.gz`, stray assets) is silently dropped.
TARGET_KEYS = {
    "linux-glibc-amd64",
    "linux-glibc-arm64",
    "linux-musl-amd64",
